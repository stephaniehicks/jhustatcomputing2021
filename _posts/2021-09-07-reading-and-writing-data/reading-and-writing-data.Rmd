...
> "When writing code, you're always collaborating with future-you; 
> and past-you doesn't respond to emails". ---*Hadley Wickham*
...
```{r data-meme, echo = FALSE, fig.cap = "Meme: that variable should be cat-egorical", preview=TRUE}
knitr::include_graphics("https://pbs.twimg.com/media/DKrgcLMWkAEYgUO?format=jpg&name=small")
```

[[Source](https://pbs.twimg.com/media/DKrgcLMWkAEYgUO?format=jpg&name=small)]
...
function being used which explicitly tells R to change the absolute path 
...
```{r figsetwd, echo=FALSE, fig.cap='Artwork by Allison Horst on on the setwd function', out.width = '60%', fig.align='center'}
...
working directory path and the files available using `list.files()`  
...
OK so our current location is in the reading and writing lectures sub-folder 
of the `jhustatcomputing2021` course repository. Let's try using the 
`here` package. 
...
_relative_ path (relative to the `.Rproj` file in our `jhustatcomputing2021` 
repository. We also see there is are two `.csv` files in 
the `data` folder. We will learn how to read those files into R in the next section. 

```{r here-pkg, echo = FALSE, fig.cap = "Artwork by Allison Horst on the here package"}
knitr::include_graphics("https://github.com/allisonhorst/stats-illustrations/raw/master/rstats-artwork/here.png")
```
...
One last thing. If you want to download a file, one way to use the 
...
* `save.image()`: short for 'save my current workspace; while this _sounds_ nice, it's not terribly useful for reproducibility (hence not suggested); it's also what happens when you try to quit R and it asks if you want to save your work space. 
...
Now, there are of course, many R packages that have been developed to read
...
* `stringsAsFactors`, should character variables be coded as factors?
  This defaults to `FALSE`. However, back in the "old days", it defaulted to `TRUE`. The reason for this was because, if you had
  ...
  data that is text data and they do not always represent categorical
...
* Set `nrows`. This does not make R run faster but it helps with memory
...
objects residing in your workspace. One situation where it is
particularly important to understand memory requirements is when you
are reading in a new dataset into R. Fortunately, it is easy to make a
...
required to store this data frame? 

Well, on most modern computers
...
2. Why was the arguement `stringsAsFactors=TRUE` historically used?
