...
Plots are made up of _aesthetics_ (e.g. size, shape, color) and _geoms_ (e.g. points, lines). Factors play an important role for indicating subsets of the data (if they are to have different properties) so they should be __labeled__ properly. 
...
```{r,fig.cap="Plot of engine displacement and highway mileage using the mtcars dataset"}
qplot(x = displ, y = hwy, data = mpg)
```

It has a _very_ similar feeling to `plot()` in base R. 
...
**Example**: Let's try mapping colors in another dataset, namely the [palmerpenguins](https://allisonhorst.github.io/palmerpenguins/) dataset. 
...
```{r tidy-data, echo = FALSE, fig.cap = "Palmer penguins", out.width = '60%', fig.align='center'}
...
<details> <summary> Click here for case study practicing the `qplot()` function.</summary>
