...
```{r rallisonhorst, echo=FALSE, fig.cap='Artwork by Allison Horst on learning R', out.width = '60%', fig.align='center'}
...
R has maintained the original S philosophy (see box below), which is that **it provides a
...
is considerable activity on web sites like Stack Overflow, Twitter [#rstats](https://twitter.com/search?q=%23rstats) and [Reddit](https://www.reddit.com/r/rstats/). 
...
- There are over 10,000 packages on CRAN that have been developed by
...
be stored in physical memory (though this is increasingly not true anymore). 
This is in part due to the scoping rules
...
```{r rstudio-screenshot, echo = FALSE, fig.cap = "A screenshot of the RStudio integrated developer environment (IDE) -- aka the working environment."}
knitr::include_graphics("https://github.com/njtierney/rmd4sci/raw/master/figs/rstudio-screenshot.png")
```
...
```{r packageinstall, eval = FALSE, echo = TRUE}
