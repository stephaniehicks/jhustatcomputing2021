...
and less of a problem as most laptops come with >4-16Gb 
of memory and it is easy to get instances on cloud providers 
...
Let's say you are able load part of the data into the RAM on your
machine (in-memory).
...
to see what is inside the files, but eventually you will likely need a 
different strategy. 
...
You have to keep in mind that you will 
...
**Question**: How can we use the `DBI::dbConnect()` function with 
...
**Question**: Next, let's use the `dplyr::tbl()` function returns 
...
**Note**: `dplyr::collect()` forces a computation of a database
